				...
				bool isBlockedLine(const PositionData &position, const Square &from, const Square &to)
				{
					...
					if (from.col() == to.col()) // line
					{
						...
						for (char row = row_low+1; row < row_high; ++row)
						{
							  ...
							  {
//								  std::cout << "line blocked at " << from.col() << row << std::endl;
								  return true;
							  }
						}
						return false;
					...
					else if (from.row() == to.row()) // row  
					{
						...
						for (char col = col_low+1; col < col_high; ++col)
						{
							  ...
							  {
//								  std::cout << "row blocked at " << col << from.row() << std::endl;
								  return true;
							  }
						}
						return false;
					...
					else if (from.col()-from.row() == to.col()-to.row()) // diagonal 1:  a1-b2-c3-...
					{
						...
						for (char col = col_low+1; col < col_high; ++col)
						{
							  ...
							  {
//								  std::cout << "diag1 blocked at " << col << row <<std::endl;
								  return true;
							  }
							  row++;
						}
					...
					else if (from.col()+from.row() == to.col()+to.row()) // diagonal 2:  h1-g2-f3-...
					{
						...
						for (char col = col_low+1; col < col_high; ++col)
						{
							  if (position.pieceAt(col, row) != Piece::Null())
							  {
//								  std::cout << "diag2 blocked at " << col << row <<std::endl;
								  return true;
							  }
							  row--;
						}
...
//					std::cout << "------------------------------------------------" << std::endl;
//					std::cout << "evaluating pin for " << from << " - " << to << std::endl;
...
//					std::cout << "king goes N" << std::endl;
					for (int i=king_itr->row()+1; i<'8'; ++i)
					{
						...
						{
//							std::cout << "king meets opponent " << p << " at " << static_cast<char>(king_itr->col()) << static_cast<char>(i) << std::endl;
							return true;
						}
						else if (p != Piece::Null())
						{
//							std::cout << "king meets own " << p << " at " << static_cast<char>(king_itr->col()) << static_cast<char>(i) << std::endl;
							break;
						}
					}
					// king goes S
//					std::cout << "king goes S" << std::endl;
...
//					std::cout << "king goes E" << std::endl;
					for (int i=king_itr->col()+1; i<='h'; ++i)
					{
						...
						if ((p == his_piece) || (p == his_queen))
						{
//							std::cout << "king meets opponent " << p << " at " << static_cast<char>(i) << static_cast<char>(king_itr->row()) << std::endl;
							return true;
						}
						else if (p != Piece::Null())
						{
//							std::cout << "king meets own " << p << " at " << static_cast<char>(i) << static_cast<char>(king_itr->row()) << std::endl;
							break;
						}
					}
					// king goes W
//					std::cout << "king goes W" << std::endl;
					for (int i=king_itr->col()-1; i>='a'; --i)
					{
						his_piece = (position.sideToMove == white ? Piece::BlackRook() : Piece::WhiteRook());

//						Piece p = position.pieceAt(i, from.row());
						Piece p = position.pieceAt(i, king_itr->row());
...
//							std::cout << "king meets own " << p << " at " << static_cast<char>(i) << static_cast<char>(king_itr->row()) << std::endl;
					...
					// diagonal lines
					// king goes NE 
//					std::cout << "king goes NE" << std::endl;
					int i,j;
					for (i=king_itr->col()+1, j=king_itr->row()+1; ; ++i, ++j)
					{
						if ((i > 'h') || (j > '8')) // hit a board edge
						{
//							  std::cout << "king meets the edge at " << static_cast<char>(i) << static_cast<char>(j) << std::endl;
							  break;
						}
						...
						if ((p == his_piece) || (p == his_queen))
						{
//							std::cout << "king meets opponent " << p << " at " << static_cast<char>(i) << static_cast<char>(j) << std::endl;
							return true;
						}
						else if (p != Piece::Null())
						{
//							std::cout << "king meets own " << p << " at " << static_cast<char>(i) << static_cast<char>(j) << std::endl;
							break;
						}
					}

					// king goes NW
//					std::cout << "king goes NW" << std::endl;
					for (i=king_itr->col()-1, j=king_itr->row()+1; ; --i, ++j)
					{
...
//							std::cout << "king meets own " << p << " at " << static_cast<char>(i) << static_cast<char>(j) << std::endl;
					...
					// king goes SE
//					std::cout << "king goes SE" << std::endl;
					for (i=king_itr->col()+1, j=king_itr->row()-1; ; ++i, --j)
					{
...
//							std::cout << "king meets own " << p << " at " << static_cast<char>(i) << static_cast<char>(j) << std::endl;
					...
					// king goes SW
//					std::cout << "king goes SW" << std::endl;
					for (i=king_itr->col()-1, j=king_itr->row()-1; ; --i, --j)
					{
		...
		class KnightMovementRule : public PieceMovementRule
		{
			public:
				bool checkLegalMovement(const PositionData &position, const Square &from, const Square &to)
				{
				      ...
				      {
//						std::cout << "AAA: " << from.col() << from.row() << "-" << to.col() << to.row() << std::endl;
						return false;				      
				      }
				}
		};
	...
	void PositionData::fillFromSquare(pgn::Ply &ply)
	{
		...
		PieceMovementRule *rule;
//			std::cout << "ply.piece(): " << piece.letter() << std::endl;
		switch (piece.letter())
		{
			case 'P':
				if (sideToMove == white)
...
//		ply = pgn::Ply(ply.str(), from);
	...
	// - castling field (partially supported)
	// - en passant field (unsupported)
	//
	class Fen
	{
		public:
			...
			Fen(const PositionData &position)
			{
				...
				castling_ = castlingEvaluation(position); // partially supported
		...
		private:
			...
			{
			      position.squares.push_back(Square(col, row, piece));
...
//			hdata->squares.push_back(pgn::Square('a'+j, i+1));
}
...
//		std::cout << "update: set empty " << from << ", set " << piece.letter() << " to " << to << std::endl;
	}
